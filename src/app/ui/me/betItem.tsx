export default function BetItem({ side, bettingline, wager, heading, subheading, payoutrate }: { side: string, bettingline: number, wager: number, heading: string, subheading: string, payoutrate: number}) {
    const sideString = (side === 'o') ? 'Over' : (side === 'u') ? 'Under' : (side === 'h') ? 'Hit' : (side === 'm') ? 'Miss' : '';
    const payoutNum = payoutrate * wager;
    return (
        <div className="rounded-lg bg-gray-300 py-5 px-12 flex flex-col gap-6 drop-shadow-md">
