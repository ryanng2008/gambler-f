...
export function RegularOption({ open, onOpen, optionItem, balance, type }: { open: boolean, onOpen: any, optionItem: OptionType, balance: number, type: 'ou' | 'hm' }) {
    ...
    async function onSubmit() {
        ...
        if(betPlacement.success) {
            console.log('Success!')
            setMessage('Success!');
        } else if(betPlacement.message === 'No Money'){
            console.log('Failed')
            ...
            <div className={`overflow-hidden duration-1000 ease-in-out ${open ? 'max-h-[600px]' : 'max-h-0'}`}>
                {
                    message 
                    ?
                    <div className='text-center mx-auto my-12'><p className=''>{message}</p></div> 
                    : 
                    <div className='px-8 py-6 flex flex-col gap-4'>
                        ...
                        <div className='space-y-1'>
                            ...
                            range={(balance > Number(optionItem.minbet)) ? [Number(optionItem.minbet), Math.min(Number(optionItem.maxbet), balance)] : [0, 0]}
                            step={1}
                            />
                        </div>
